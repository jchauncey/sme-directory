...
- **Super admin (portal-level)**: `User.isSuperAdmin` is a portal-wide privilege independent of any group membership. Page guard: `requireSuperAdmin()` in [src/lib/auth.ts](src/lib/auth.ts) (404s for non-admins so the surface isn't advertised). Server-side gate: `assertSuperAdmin(userId)` from [src/lib/admin-auth.ts](src/lib/admin-auth.ts) — call this at the top of every privileged operation since the JWT can be stale after a demotion. Operations live in [src/lib/admin.ts](src/lib/admin.ts) and every mutation writes to `AdminAuditLog` via [src/lib/admin-audit.ts](src/lib/admin-audit.ts). The `/admin` route segment surfaces these (groups, users, content, audit log). Bootstrap: `prisma/seed.ts` flags alice in dev; in prod, run `UPDATE "User" SET "isSuperAdmin" = true WHERE email = '...';` manually.
